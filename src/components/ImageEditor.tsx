...
import type { Point, Dimensions, AspectRatio, SliceMode } from "../types";
...
const ImageEditor: React.FC<ImageEditorProps> = ({
  ...
  const [canvasDims, setCanvasDims] = useState<Dimensions>({
    width: 1200,
    height: 600,
  });
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 pointer-events-none opacity-0 hover:opacity-100 transition-opacity">
          <span className="bg-black/60 text-white text-xs px-2 py-1 rounded backdrop-blur-sm">
            {Math.round(canvasDims.width)} x {Math.round(canvasDims.height)} •{" "}
            {sliceMode === "grid" ? "2x2" : "4x Stack"}
          </span>
        </div>
      </div>
